...
- **Memory Harvester * — periodically snapshots all `MEM_COMMIT` + executable (`PAGE_EXECUTE_*`) regions outside the PE image and dumps them as separate `.bin` files, catching VMProtect stubs, JIT-allocated shellcode, and self-modifying code that lives outside the original section layout.
